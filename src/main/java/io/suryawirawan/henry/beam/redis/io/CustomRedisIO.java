...
public final class CustomRedisIO {
  ...
  public abstract static class Write extends PTransform<PCollection<KV<String, String>>, PDone> {
    ...
    public enum Method {
      ...
      /**
       * Use LPUSH + LTRIM command. Insert value at the head of the list stored at key. If key does not
       * exist, it is created as empty list before performing the push operations. When key holds a
       * value that is not a list, an error is returned. Trim to capped collection size.
       */
      LTRIM,

      /**
       * Use RPUSH + LTRIM command. Insert value at the tail of the list stored at key. If key does not
       * exist, it is created as empty list before performing the push operations. When key holds a
       * value that is not a list, an error is returned. Trim to capped collection size, starting from the end.
       */
      RTRIM,
    ...
    @Nullable
    abstract Long cappedCollectionSize();
    ...
    abstract static class Builder {
      ...
      abstract CustomRedisIO.Write.Builder setCappedCollectionSize(Long collectionSize);
    ...
    public CustomRedisIO.Write withCappedCollectionSize(Long collectionSize) {
      checkArgument(collectionSize != null, "collectionSize can not be null");
      checkArgument(collectionSize > 0, "collectionSize can not be negative or 0");
      return builder().setCappedCollectionSize(collectionSize).build();
    }
    ...
    private static class WriteFn extends DoFn<KV<String, String>, Void> {
      ...
      private void writeRecord(KV<String, String> record) {
        ...
        Long cappedCollectionSize = spec.cappedCollectionSize();

        if (Method.APPEND == method) {
          writeUsingAppendCommand(record);
        } else if (Method.SET == method) {
          writeUsingSetCommand(record, expireTime);
        } else if (Method.LPUSH == method || Method.RPUSH == method || Method.LTRIM == method || Method.RTRIM == method) {
          writeUsingListCommand(record, method, expireTime, cappedCollectionSize);
        }
      }
      ...
      private void writeUsingListCommand(
          KV<String, String> record, Method method, Long expireTime, Long collectionSize) {
        ...
        } else if (Method.LTRIM == method) {
          pipeline.lpush(record.getKey(), record.getValue());
          pipeline.ltrim(record.getKey(), 0, collectionSize);
        } else if (Method.RTRIM == method) {
          pipeline.rpush(record.getKey(), record.getValue());
          pipeline.ltrim(record.getKey(), 0, -collectionSize);
        }
